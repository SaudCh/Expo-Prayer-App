...
export default function App(props) {
  ...
  const zawalTime = (time) => {
    ...
    var mint;
    var zone = 'AM';
    hour = parseInt((time.substring(0, 2))) + 6
    if (hour >= 12) {
      zone = 'PM'
    }
    if (hour != 12) {
    ...
    return ("Starts " + string1)
  }
  const fajarEnds = (time) => {
    var hour;
    var mint;
    var zone = 'AM';
    hour = parseInt((time.substring(0, 2)))

    mint = parseInt(time.substring(3, 5)) - 1
  ...
  const zawalEnds = (time) => {
    ...
    return "Ends   " + hour + ":" + mint + " " + zone
  }

  const duharEnds = (time) => {
    var hour;
    var mint;
    var zone = 'AM';
    hour = parseInt((time.substring(0, 2)))
    ...
    hour = hour % 12
    mint = parseInt(time.substring(3, 5)) - 10
    if (mint < 0) {
      mint = 60 + mint
      hour = hour - 1
    }
  ...
  const asrEnds = (time) => {
    ...
    hour = hour % 12
    mint = parseInt(time.substring(3, 5)) - 15
    if (mint < 0) {
      mint = 60 + mint
      hour = hour - 1
    }
  ...
  async function load() {
    try {
      ...
      const apiURL = `${PRAYER_API_URL}city=${city}&country=Pakistan&method=1&school=${school}`
      ...
      <View style={styles.container}>
        ...
        <ScrollView >
          ...
          <View style={{ ...styles.timing }}>
            ...
            <Text style={{ ...styles.text }}>Starts {timeFormat(data.data[date].timings.Fajr)}</Text>
            <Text style={{ ...styles.text }}>Ends   {fajarEnds(data.data[date].timings.Sunrise)}</Text>
          </View>
          <View style={{ ...styles.timing }}>
          ...
          <View style={{ ...styles.timing }}>
            ...
            <Text style={{ ...styles.text }}>{zawalEnds(data.data[date].timings.Dhuhr)}</Text>

          </View>
          <View style={{ ...styles.timing }}>
            <Text style={{ ...styles.text }}>Duhar</Text>
            <Text style={{ ...styles.text }}>Starts {timeFormat(data.data[date].timings.Dhuhr)}</Text>
            <Text style={{ ...styles.text }}>{duharEnds(data.data[date].timings.Asr)}</Text>
          </View>
          <View style={{ ...styles.timing }}>
            <Text style={{ ...styles.text }}>Asr</Text>
            <Text style={{ ...styles.text }}>Starts {timeFormat(data.data[date].timings.Asr)}</Text>
            <Text style={{ ...styles.text }}>{asrEnds(data.data[date].timings.Sunset)}</Text>
          </View>
          <View style={{ ...styles.timing }}>
          ...
          <View style={{ ...styles.timing }}>
            ...
            <Text style={{ ...styles.text }}>Starts {timeFormat(data.data[date].timings.Maghrib)}</Text>
            <Text style={{ ...styles.text }}>{magribEnds(data.data[date].timings.Sunset)}</Text>

          </View>
          <View style={{ ...styles.timing }}>
            <Text style={{ ...styles.text }}>Ishaa</Text>
            <Text style={{ ...styles.text }}>Starts {timeFormat(data.data[date].timings.Isha)}</Text>
            <Text style={{ ...styles.text }}>Ends   {timeFormat(data.data[date].timings.Imsak)}</Text>
          </View>
          <View style={{ ...styles.timing }}>
            <Text style={{ ...styles.text }}>Mid Night</Text>
            <Text style={{ ...styles.text }}>{midNight(data.data[date].timings.Midnight)}</Text>
          </View>
